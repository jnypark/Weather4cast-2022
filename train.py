...
from models.rainunet_lightning import UNet_Lightning as RainUNetModel




method_maps = {
    'RainUNetModel':RainUNetModel,
    'UNetModel':UNetModel,
}
...
class DataModule(pl.LightningDataModule):
    ...
    def __init__(self, params, training_params, mode):
        ...
        if mode in ['train']:
            ...
            print(f"Training validation dataset size: {len(self.val_ds)}")
        if mode in ['val']:
...
def get_trainer(gpus,params):
    ...
    model_name=params['experiment']['name']
    checkpoint_callback = ModelCheckpoint(monitor='val_precision', save_top_k=10, mode='max',save_last=True,
                                          filename=f'{model_name}_'+'{epoch:02d}-{val_precision:.6f}-{val_iou:.6f}')
...
##        ddpplugin = DDPPlugin(find_unused_parameters=True)
    ...
    project="META3/w4c",
            api_key=params['experiment']['neptune'],
            tags=params['experiment']['tags'],
    ...
    if params['train']['early_stopping']: 
        early_stop_callback = EarlyStopping(monitor="val_loss_epoch",
                                            ...
                                            mode="min")
        if params['train']['StochasticWeightAveraging']==True: 
            print('==StochasticWeightAveraging Mode !!== ')
            from pytorch_lightning.callbacks import StochasticWeightAveraging
            callback_funcs = [checkpoint_callback, early_stop_callback,StochasticWeightAveraging(swa_lrs=1e-2)]
        else:
        ...
        if params['train']['StochasticWeightAveraging']==True:
            print('==StochasticWeightAveraging Mode !!== ')
            from pytorch_lightning.callbacks import StochasticWeightAveraging
            callback_funcs = [checkpoint_callback,StochasticWeightAveraging(swa_lrs=1e-2)]
        else:
    ...
    trainer = pl.Trainer(devices=gpus, max_epochs=max_epochs,
                         ...
                         accelerator="gpu",
                         callbacks=callback_funcs,logger=logger,
...
def train(params, gpus, mode, checkpoint_path, model=RainUNetModel): 
...
def update_params_based_on_args(options):
    ...
    params['experiment']['name'] = options.model_name
    return params
...
def set_parser():
    ...
    parser.add_argument( "--model_name", type=str, default='RainUNetModel', 
                         help="Set the name of the experiment UNetModel ")
...
def main():
    ...
    model = method_maps[options.model_name]
    train(params, options.gpus, options.mode, options.checkpoint,model=model)
