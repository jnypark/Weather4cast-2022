...
class DownConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, dropout_rate, pooling=True, planar=False, activation='relu',
        ...
        self.dwconv = conv3(
            self.in_channels, self.out_channels, planar=True, dim=dim, padding=padding
        )
        self.proj = conv3(
            self.in_channels, self.out_channels, kernel_size=1,planar=True, dim=dim) #, padding=padding)
        
        self.proj = nn.Conv3d(self.in_channels, self.out_channels, kernel_size = 1)
        self.dwconv = nn.Conv3d(self.out_channels, self.out_channels, kernel_size = (1,3,3), stride=1, padding=(0,1,1))
        self.dwdconv = nn.Conv3d(self.out_channels, self.out_channels, kernel_size = (1,7,7), stride=1, padding=(0,9,9),dilation=3)
        self.conv_ = nn.Conv3d(self.out_channels, self.out_channels, kernel_size = (3,1,1),padding=(1,0,0))
...
class UpConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels,
        ...
        #self.act1 = nn.GELU()
        self.act2 = get_activation(activation)
        self.proj = nn.Conv3d(2*self.out_channels, self.out_channels, kernel_size = 1)
        self.dwconv = nn.Conv3d(self.out_channels, self.out_channels, kernel_size = (1,3,3), stride=1, padding=(0,1,1))
        self.dwdconv = nn.Conv3d(self.out_channels, self.out_channels, kernel_size = (1,7,7), stride=1, padding=(0,9,9),dilation=3)
        self.conv_ = nn.Conv3d(self.out_channels, self.out_channels, kernel_size = (3,1,1),padding=(1,0,0))
...
class UNet(nn.Module):
    ...
    Args:
        ...
        normalization: Type of normalization that should be applied at the end
            ...
            - ' ' for batch normalization (default)
    ...
    def forward(self, x):
        ...
        """
        if self.centercrop:
            x = x[:,:,:,63:-63,63:-63] # centor
            B, C, T, h,w = x.shape
            x = x.view(B*C, T, h,w)
            x = F.interpolate(x, scale_factor=2, mode='bilinear', align_corners=False)
            x = x.view(B,C, T,h*2,h*2)
        encoder_outs = []
